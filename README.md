...
2) Press Ctrl+Shift+P (Windows) or Cmd+Shift+P (Mac) and click on "Python: Select Interpreter"
   - choose the one with `.venv\Scripts\python.exe` (Windows) or `.venv/bin/python` (Mac) in its name
...
4) Run `pip install --upgrade pip; if ($?) { pip install -r requirements.txt }` (Windows) or `pip install --upgrade pip && pip install -r requirements.txt` (Mac) to install the libraries into the virtual environment

You are now in the virtual environment! This is where you run the code with the correct libraries
